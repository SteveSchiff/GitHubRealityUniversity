 ...
 */
public class GroupSurveysLowerRightSidePanel extends RoundPanel implements
		GuiInterface {
	...
	public GroupSurveysLowerRightSidePanel() {
		...
		if (Controller.getControllerInstance().getGroup().getID() > 0) {
			...
			// scrollPane object.
			scrollPane.setViewportView(surveysPanel); // insert surveysPanel
														// into scrollPane to
														// allow scrolling for
														// long survey lists
			...
			surveysPanel.setLayout(gbl_surveysPanel); // insert gbl_pnlSurveys
														// into surveysPanel
			surveysPanel.setBackground(FOOTPANEL_BACKGROUND_INDIANRED); // Indian
																		// Red
																		// by
																		// Steve
			...
			if (listOfSurveys.size() > 0) { // inner if statement - ends on line
				...
				for (int row = 0; row < listOfSurveys.size(); row++) { // ends
																		// on
																		// line
																		// 359
					...
					JLabel lblName = new JLabel((row + 1) + ". " + lastName
							+ ", " + firstName, JLabel.LEFT);
				...
				printButton.addMouseListener(new MouseListener() { // inner
																	// class
							public void mouseClicked(MouseEvent arg0) {
								...
								System.out
										.println("*************************************");
								System.out
										.println("*   -- Print All Button Clicked--   *");
								System.out
										.println("*                                   *");
								System.out
										.println("*************************************");

								List<Survey> listOfSurveys = Controller
										.getControllerInstance()
										.getSurveysList();
								System.out.println("listSurveys.size() is "
										+ listOfSurveys.size());

								if (listOfSurveys.size() > 0) {

									JFrame[] imagePages = new JFrame[listOfSurveys
											.size()];
									for (int i = 0; i < listOfSurveys.size(); i++) {//

										ViewSurvey viewSurvey = new ViewSurvey(
												listOfSurveys.get(i));
										imagePages[i] = viewSurvey
												.paperFrameSetUp();
										viewSurvey.dispose();
